...
void StandardTestStatDistributionDemo(const char* infile = "",
  ...
  ROOT::Math::MinimizerOptions::SetDefaultPrintLevel(1);
   ...
   RooArgSet params(*mc->GetNuisanceParameters(),*mc->GetParametersOfInterest());
     ...
     double err_guess = firstPOI->getError();
     double prec = 0.002;
     double nll_min = nll->getVal();
     double firstPOI_best = firstPOI->getVal();
     double tot_hi = findSigma(nll, nll_min, firstPOI, firstPOI_best+err_guess, firstPOI_best, +1, prec);
     double plcUpperLimit = firstPOI_best+tot_hi;
     // -------------------------------------------------------
  ...
  ROOT::Math::MinimizerOptions::SetDefaultStrategy(0);
   ...
   TF1* f = new TF1("f",Form("2*ROOT::Math::chisquared_pdf(2*x,%d,0)",1),min,max);
   f->Draw("same");
