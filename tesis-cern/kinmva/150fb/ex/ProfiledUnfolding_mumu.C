...
void ProfiledUnfolding_mumu(int nrBins = 12)
{
  //Histos were already made by ProfiledUnfolding_makeHists.C. Let's open them up and use them
  TFile* file = new TFile("hists/ProfiledUnfolding_mumu.root");
  ...
  //Randomize the pseudo-data in order to introduce some level of fluctuation
  for (int i=0;i<h_yZ_reco->GetNbinsX();i++)
  {
    double content = h_yZ_reco->GetBinContent(i+1);
    content = rndm.Poisson(content);
    h_yZ_reco->SetBinContent(i+1,content);
    h_yZ_reco->SetBinError(i+1,sqrt(content));
  }
  
//--------------------

//create the histfactory model
  RooStats::HistFactory::Measurement meas("ProfiledUnfolding_mumu", "ProfiledUnfolding_mumu"); 
  meas.SetOutputFilePrefix("workspaces/ProfiledUnfolding_mumu/tut");
...
//create the SR
  RooStats::HistFactory::Channel chan_mumu("mumu_yZ");
  chan_mumu.SetData(h_yZ_reco);
...
//add the signal samples
    RooStats::HistFactory::Sample sample(Form("yZ_acceptance_template_%i_mumu",i));
    sample.SetNormalizeByTheory(false);
    sample.SetHisto(h_yZ_reco_i[i]);
    sample.AddNormFactor(yZ_XS);
    chan_mumu.AddSample(sample);
  }

  RooStats::HistFactory::Sample sample_nonFid("ZnonFid_mumu");
  sample_nonFid.SetHisto(h_yZ_reco_nonFid);
  chan_mumu.AddSample(sample_nonFid);

//add the single region to the measurement
  meas.AddChannel(chan_mumu);
