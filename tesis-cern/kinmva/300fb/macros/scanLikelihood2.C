...
void scanLikelihood2(string version, string parName="mu", double range=4, int nrPoints=20)
{
  ...
  graph->GetXaxis()->SetLimits(-40,40);
  ...
  l.SetLineWidth(2);
  l.SetLineColor(kRed);
  l.DrawLine(-34,1,34,1);
  l.SetLineColor(kBlue);
  l.DrawLine(-34,4,34,4);
  l.SetLineColor(kGreen+4);
  l.DrawLine(-34,9,34,9);

TLatex lat ;
lat.SetTextSize(0.03);
lat.DrawLatex(4.5,1,"1#sigma");

TLatex lat2;
lat2.SetTextSize(0.03);
lat2.DrawLatex(4.5,4,"2#sigma");
lat2.DrawLatex(4.5,9,"3#sigma");
cs->SaveAs("Likelihood-kt-1.png");

///////////////////////////////////////////////////////
double   pi=3.1415926;
TF1 *fa1 = new TF1("fa1","((1/sqrt(2.*pi))*(1/sqrt(x))*exp(-x/2.))",0,8);
fa1->SetLineColor(kBlue);
TCanvas *tm = new TCanvas("cs","cs",100,100,1200,900);
fa1->SetTitle();
fa1->GetXaxis()->SetTitle("t_{#mu}");
fa1->GetYaxis()->SetTitle("f(t_{#mu})");
fa1->Draw();
tm->SaveAs("tmu-kt-1.jpg");

/////////////////////////////////////////////////////////
TCanvas *tm1 = new TCanvas("tm1","tm1",100,100,1200,900);
TF1 *fa2 = new TF1("fa2","((1/sqrt(2.*pi))*(1/sqrt(x))*exp(-x/2.))",0,8);
...
TArrow *ar2 = new TArrow(0,-0.05,2,-0.05,0.01,"<>");
ar2->SetLineColor(4);
ar2->Draw();
lat2.SetTextSize(0.03);
lat2.SetTextColor(4);
lat2.DrawLatex(1.32,-0.1,"95.4%");


tm1->SaveAs("nos-kt-1.png");
